// Write a real-valued function RadToDeg(R) that converts the angle value R
// in radians into the one in degrees (R is a real number, 0 ≤ R < 2·π). Note that
// 180° = π radians and use 3.14 for a value of π. Using this function, convert five
// given angles from radians into degrees.

// Function to convert an angle value from radians to degrees
function radToDeg(R) {
    const PI = 3.14;
    return (R * 180) / Math.PI;
}

// Example usage:
const anglesInRadians = [0, Math.PI / 6, Math.PI / 4, Math.PI / 2, Math.PI];
const anglesInDegrees = anglesInRadians.map(radToDeg);
console.log("Angles in degrees:", anglesInDegrees);
